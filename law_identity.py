...
# السنة تُلتقط بإحدى صيغ رسمية واقعية (ف٣ — 2026-09-19، قياساً على 291
# وثيقة بلا هوية بقاعدة المالك):
#   «لعام 2010» / «للعام 2010» / «لسنة 2011» / «عام 2001» / «سنة 2001»
#   «تاريخ 22/6/1949» أو «تاريخ 1949/6/22»  ← السنة آخر/أول مقطع رباعي
...
# المسافة بين نوع الصك وكلمة «رقم» تتسع لعنوان الموضوع («القانون الأساسي
# للعاملين في الدولة رقم 50») — 60 حرفاً بلا أرقام: أطول من ذلك إحالة لا
# عنوان. رقم الصك يقبل «/84/» و«(17)».
# الفجوة لا تحوي نوع صك آخر: في «القانون الجنائي (الصادر بالمرسوم
# التشريعي رقم 148/1949)» النوع الأقرب لكلمة «رقم» هو نوع الصك الفعلي.
_GAP = rf"(?:(?!{_TYPE_ALT})[^\d]){{0,60}}?"
LAW_ID_RE = re.compile(
    rf"({_TYPE_ALT})"
    rf"{_GAP}رقم\s*[/\(]?\s*(?P<num>{_NUM})\s*[/\)]?"
    rf"{_YEAR_PART}",
)
...
def extract_law_identity(title: str, text: str) -> dict:
    ...
    for idx, haystack in enumerate(haystacks):
        ...
        if not m and idx == 0:
            # احتياطان بلا كلمة «رقم» — العنوان حصراً (النص يحمل إحالات
            # صليبية فتكون هوية زائفة): «148 لعام 1949» ثم «98/1961».
            m = (LAW_ID_NO_RAQM_RE.search(haystack)
                 or LAW_ID_SLASH_RE.search(haystack))
        if not m:
            continue
