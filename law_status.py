...
PARTIAL_OBJECT_RE = re.compile(
    ...
    r"(?:ب|بأحكام\s+|باحكام\s+|نص\s+|نصوص\s+|أحكام\s+|احكام\s+)?"
    r"(?:ال)?(?:مادة|مادتين|مادتان|مواد|فقرة|فقرتين|فقرات|بند|بندين|بنود|فصل|فصول|باب|جدول|عبارة)"
    r"|(?:يلغ[يى]|تلغ[يى])\s+(?:كل|جميع|كافة)?\s*(?:نص|حكم)\s+مخالف\s+(?:في|من)\b")
# قِيس repealed4 (بنود، 2026-09-19): «تلغى نصوص المواد /7 و14/ من م.ت 81»،
# «ينهى العمل بأحكام المادتين /1 و2/ من القانون 4»، «يلغى كل نص مخالف في
# م.ت 35» — ثلاثتها كانت تعلّم الصك كله «ملغى».
...
def classify_reference(ref: dict) -> str:
    ...
    if re.search(r"(?:أحكام|احكام|نصوص)\s+(?:ال)?مخالف[^.؛]{0,80}الواردة\s+في\s+(?:ال)?(?:قانون|مرسوم|قرار|نظام)",
        ...
        return "amend"
    # ── المقطع المحلي (repealed5 على متن بنود، 2026-09-20): ما بين آخر صك
    # مذكور قبل هذه الإشارة وبينها. الفعل في أول الجملة لا يمتد إلى صك
    # ورد وصفاً لصك آخر أو جزءاً منه:
    #   «يلغى القانون X المتضمن تعديل المادة 790 من القانون المدني الصادر
    #    بموجب [م.ت 84/1949]» → المدني ليس ملغى (وصف)؛
    #   «ينهى العمل بـ م.ت 27 والفقرة ب من المادة 12 من [القانون 30]» → تعديل؛
    #   «ولا سيما المادة 18 من [القانون 8]» → تعديل؛
    #   «وتنتقل مهامه المذكورة في [م.ت 208]» → استشهاد؛
    #   «ويعاد العمل بـ [م.ت 190]» → استشهاد (إعادة نفاذ لا إلغاء).
    parts_ = re.split(r"(?:لعام|لسنة|سنة|تاريخ|المؤرخ في)\s*/?\s*\d{2,4}\s*/?", before)
    local = parts_[-1]
    verb_before = bool(REPEAL_RE.search(before) or AMEND_RE.search(before))
    if verb_before and re.search(
            r"(?:ال)?(?:مادة|مادتين|مادتان|مواد|فقرة|فقرتين|فقرتان|فقرات|بند|بنود)"
            r"\s*[^.؛]{0,40}?\bمن\s*(?:أحكام\s+|قانون\s+|نظام\s+|ال)?[^.؛]{0,60}$", local):
        return "amend"
    # «…ويعد [م.ت 44/2005] والقانون 35/2007 معدلين حكماً» — الفعل «يعد»
    # يفتح جملة جديدة: تعديل لا امتداد للإلغاء السابق (repealed6).
    if re.search(r"\bو?(?:يعد|تعد|ويعتبر|وتعتبر)\s*$", local) or \
            re.search(r"\bمعدل(?:ين|ة|اً|ا)?\s+حكم", after[:60]):
        return "amend"
    if re.search(r"(?:المذكور[ةه]?\s+في|الوارد[ةه]?\s+في|المنصوص\s+علي\w*\s+في|"
                 r"المشار\s+إلي\w*\s+في|يعاد\s+العمل\s+ب|تعاد\s+العمل\s+ب)[^.؛]{0,50}$", local):
        return "cite"
    # «…المتضمن/بموجب [صك]» بعد صك سابق في الجملة نفسها = وصف لذلك الصك
    if len(parts_) > 1 and re.search(r"(?:بموجب|المتضمن|المحدث[ةه]?\s+ب)[^.؛]{0,50}$", local):
        return "cite"
